...
namespace Hypermarket_Admin_Management_Tool._0_View
{
    public partial class Login : Form
    {
        ...
        private void LoginToSystem()
        {
            ...
            try
            {
                if (IsStringEmptyOrNull(staffID))
                {
                ...
                else if (!mainController.IsValidUser(staffID))
                {
                    lblErrorFeedback.Text = Constant.ERROR_INVALID_USER_ID;
                    txtUserID.Focus();
                }
                else if (!mainController.VerifyUserIdAndPassword(staffID, password))
                {
                ...
                {
                    if (mainController.RequireChangingPassword(staffID))
                    {
        ...
        private void SavePasswordAndLogin()
        {
            ...
            {
                mainController.ChangePassword(staffID, txtNewPassword.Text);
